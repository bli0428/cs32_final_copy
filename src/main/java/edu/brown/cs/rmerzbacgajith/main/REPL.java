...
/**
 * REPL class for maps that handles and parses the user input and calls the appropriate methods in mapCommand
 * to return accurate output.
 *
 */
public class REPL {
  ...
  /**
   * Constructor simply instantiates a new MapCommand that handles all the commands related to maps.
   */
  public REPL() {
    mc = new MapCommand();
  }
  ...
  private void processCommand(String line) {
    ...
    if (parsed[0].equals("map")) {
      ...
      if (parsed.length > 1) {
        //Parse what user has typed after the word suggest and use method in MapCommand to autocorrect
        //for suggestions.
      ...
      //For a route command, first determine whether it is a route with intersections or coordinates command.
      ...
      //Ensure Intersection 2 is in the database using MapCommand method.
        ...
        if (n2 == null) {
          ...
          //If both intersections are not null, call route command passing in the two nodes.
          mc.route(n1, n2);
        }
        ...
        if (parsed.length == 5 && numQuotes == 0) {
          ...
          //Parse both sets of coordinates and use nearest method to find the nearest traversable nodes to these coordinates.
          try {
          double[] coords1 = { Double.parseDouble(parsed[1]),
              Double.parseDouble(parsed[2]) };
          double[] coords2 = { Double.parseDouble(parsed[3]),
